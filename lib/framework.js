...
function setRotatedGeometry(window, width, height, angle) {
    qnx.callExtensionMethod("webview.setGeometry", window, 0, 0, width, height);
}

var _self = {
    start: function (url) {
        rotationHelper.addWebview(qnx.webplatform.getController().id, function () {
            return true;
        }, setRotatedGeometry);
        ...
        webview.create(function () {
            rotationHelper.addWebview(webview.id(), function () {
                return true;
            }, setRotatedGeometry);
        ...
        overlayWebView.create(function () {
            rotationHelper.addWebview(overlayWebView.id(), function () {
                return true;
            }, setRotatedGeometry);
